...
public class ActionsByEventCategoryReport {

	public HSSFWorkbook exportList(ActionsExcelService actionsExcelService, EventCategory eventCategory) {
		...
		excelToolkit.createCell(ExcelToolkit.GENERIC, wb, row, font, 3, "Λήξη", HSSFCellStyle.ALIGN_CENTER,
				HSSFCellStyle.VERTICAL_CENTER, true, true, HSSFColor.GREY_25_PERCENT.index);
		excelToolkit.createCell(ExcelToolkit.GENERIC, wb, row, font, 4, "Περιοχή", HSSFCellStyle.ALIGN_CENTER,
				HSSFCellStyle.VERTICAL_CENTER, true, true, HSSFColor.GREY_25_PERCENT.index);
		...
		try {
			Collection<Event> events = actionsExcelService.getEventsByEventCategory(eventCategory);
			...
			for (Event event : events) {
				...
				excelToolkit.createCell(ExcelToolkit.GENERIC, wb, row, font, 1, event.getEventTitle(),
				...
				if (event.getEventLocation() != null) {
					excelToolkit.createCell(ExcelToolkit.GENERIC, wb, row, font, 4, event.getEventLocation(),
